...
const useBookmarks = (seekTo, url) => {
  ...
  const getBookmarkSequenceString = useCallback(
    async (sequenceString, lyricsUrl, paragraphMode) => {
      try {
        // Early return if no bookmark sequence string exists
        if (!sequenceString || !lyricsUrl) {
          return null;
        }

        // Fetch LRC data
        const data = await fetchLRCData(lyricsUrl);
        ...
        logMessage(
          `Found bookmark position: ${matchingItem.start}s for sequence ${sequenceNumber}`
        );
